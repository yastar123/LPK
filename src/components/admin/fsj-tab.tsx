...
export function FsjCrudTab({
  ...
  const fsj = cmsStore.cms.programFsj;
  const [formData, setFormData] = useState({ ...fsj });

  const handleSave = () => {
    cmsStore.updateSection("programFsj", formData);
    showToast("Program FSJ / BFD berhasil disimpan!");
  };

  const addBenefit = () => {
    setFormData({
      ...formData,
      benefits: [...(formData.benefits || []), "Benefit baru relawan FSJ..."],
    });
  };
  ...
  const addArea = () => {
    setFormData({
      ...formData,
      areas: [...(formData.areas || []), "Bidang penempatan baru (cth: Rumah Sakit, TK, Panti)..."],
    });
  };
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900">
            Halaman Program FSJ / BFD (Freiwilliges Soziales Jahr)
          </h2>
          <p className="text-xs text-slate-500">
            Kelola info relawan sosial/medis di Jerman, uang saku bulanan, bidang penempatan, dan
            persyaratan.
          </p>
        </div>
        <button
        ...
        >
          ...
          <span>Simpan Program FSJ</span>
        </button>
      </div>

      {/* Hero Header */}
      <div className="rounded-2xl border border-slate-200 bg-white p-5 space-y-4 shadow-xs">
        <h3 className="text-sm font-bold text-slate-900">Headline & Badge Program FSJ</h3>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">
              Badge Tunjangan (Taschengeld)
            </label>
            <input
              type="text"
              value={formData.allowanceBadge || ""}
              onChange={(e) => setFormData({ ...formData, allowanceBadge: e.target.value })}
              className="w-full rounded-xl border border-slate-200 bg-slate-50/80 px-3 py-2 text-xs text-slate-900 focus:outline-none"
            />
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 space-y-3 shadow-xs">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
              ...
              <span>Bidang Penempatan Relawan</span>
            </h3>
            <button
              onClick={addArea}
          ...
          <div className="space-y-2">
            {(formData.areas || []).map((area: string, idx: number) => (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 space-y-3 shadow-xs">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
              ...
              <span>Hak & Fasilitas Relawan FSJ</span>
            </h3>
            <button
              onClick={addBenefit}
              className="inline-flex items-center gap-1 rounded-xl bg-sky-50 border border-sky-200 px-3 py-1 text-xs font-bold text-sky-700 hover:bg-sky-100"
